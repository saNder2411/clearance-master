           ...
           [:div.bg_dark
            [:div.container.py_20
             [:div.max_w_900.mx_a.c_dark.mt_14
              (feature-row {:row-cls "align_c mb_14"
                            :icon    [:img.flex_shrink_0.mr_12 {:decoding "async" :alt "icon" :width 40 :src "/svg/file-text_y.svg"}]
                            :title   "Kostenlose Besichtigung und individuelle Beratung vor Ort."})

              (feature-row {:row-cls "align_c mb_14"
                            :icon    [:img.flex_shrink_0.mr_12 {:decoding "async" :alt "icon" :width 40 :src "/svg/calendar-check_y.svg"}]
                            :title   "Schnelle Terminvereinbarung – wir passen uns Ihren Wünschen an."})

              (feature-row {:row-cls "align_c mb_14"
                            :icon    [:img.flex_shrink_0.mr_12 {:decoding "async" :alt "icon" :width 40 :src "/svg/heart-handshake_y.svg"}]
                            :title   "Persönliche Betreuung mit Herz und Kompetenz."})]]]
